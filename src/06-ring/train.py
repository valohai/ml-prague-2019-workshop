...
"""
Oh no, it won't even run! 

There are 4 new but commented lines of code that need to be uncommented 
to make this work on Horovod. That's it, making code to work with Horovod 
...
def main(_):
    ...
    # !! BroadcastGlobalVariablesHook broadcasts variables from rank 0 to all others during init. !! #
    hooks = [
    ...
    # !! Limit which GPUs should this process consider using. !! #
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    # config.gpu_options.visible_device_list = str(hvd.local_rank())

    with tf.train.SingularMonitoredSession(hooks=hooks, config=config) as sess:
        assert type(opt) == horovod.tensorflow.DistributedOptimizer
